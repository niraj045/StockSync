...
export function CreateInquiryScreen({ navigation, route }: Props) {
  ...
  const [value, setValue] = useState<InquiryRequest>({ inquiryDate: localDate(), source: 'PHONE', contactName: '', requirement: '', status: 'OPEN' });
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      const body = { ...value, contactName: value.contactName.trim(), requirement: value.requirement.trim(), phone: value.phone?.trim() || undefined, email: value.email?.trim() || undefined, notes: value.notes?.trim() || undefined };
