...
public class QuotationService implements QuotationAccess {
    ...
    private void apply(Quotation q,QuotationRequest r,boolean creating){
        ...
        q.setHeaderText(creating && (r.headerText() == null || r.headerText().isBlank()) ? trim(q.getQuotationTemplate().getHeaderText()) : r.headerText());
        q.setPartATitle(creating && (r.partATitle() == null || r.partATitle().isBlank()) ? trim(q.getQuotationTemplate().getDefaultPartATitle()) : r.partATitle());
        q.setPartAText(r.partAText());
        q.setPartBTitle(creating && (r.partBTitle() == null || r.partBTitle().isBlank()) ? trim(q.getQuotationTemplate().getDefaultPartBTitle()) : r.partBTitle());
    ...
    private void requireDraft(Quotation q){
        if(q.getStatus()==QuotationStatus.DRAFT)return;
        var auth=SecurityContextHolder.getContext().getAuthentication();
        boolean isAdmin=auth!=null&&auth.getAuthorities().stream().anyMatch(a->a.getAuthority().equals("ROLE_ADMIN"));
        if(q.getStatus()==QuotationStatus.SENT&&isAdmin)return;
        throw new BusinessRuleException("QUOTATION_IMMUTABLE","Only draft quotations (or sent quotations for admins) can be edited");
    }
